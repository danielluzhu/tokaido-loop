...
runTool("set_field", { path: `entry:${first}:city`, value: "TESTVILLE" });
check("set_field nested", city(first) === "TESTVILLE");
runTool("set_field", { path: `entry:${first}:city`, value: was });

const before = entries();
const newId: any = runTool("add_entry", { kind: "leg", after_id: first });
check("add_entry returns id", entries() === before + 1 && typeof newId.id === "number", `id ${newId.id}`);

const order1 = db.query<{ id: number }, []>("SELECT id FROM entries ORDER BY position").all().map((r) => r.id);
runTool("move_entry", { id: newId.id, direction: "up" });
const order2 = db.query<{ id: number }, []>("SELECT id FROM entries ORDER BY position").all().map((r) => r.id);
check("move_entry", order1.join() !== order2.join());

runTool("delete_entry", { id: newId.id });
check("delete_entry", entries() === before);

const noteAdd: any = runTool("edit_notes", { action: "add", id: null, heading: "T", body: "B" });
const notes = () => db.query<{ c: number }, []>("SELECT COUNT(*) AS c FROM notes").get()!.c;
const n1 = notes();
runTool("edit_notes", { action: "delete", id: noteAdd.id, heading: null, body: null });
...
const s0 = slots();
runTool("edit_slots", { entry_id: first, action: "add", label: "Eat", index: null });
const s1 = slots();
runTool("edit_slots", { entry_id: first, action: "remove", label: null, index: s1 - 1 });
check("edit_slots add/remove", s1 === s0 + 1 && slots() === s0);

// --- errors surface as thrown, so the loop can return is_error to Claude
let threw = false;
try { runTool("set_field", { path: "bogus:1:x", value: "y" }); } catch { threw = true; }
check("bad path throws", threw);
threw = false;
try { runTool("nope", {}); } catch { threw = true; }
check("unknown tool throws", threw);

check("undo tool", typeof (runTool("undo", {}) as any).ok === "boolean");
